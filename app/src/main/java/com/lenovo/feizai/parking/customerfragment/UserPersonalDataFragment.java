...
public class UserPersonalDataFragment extends BaseFragment {
            ...
            public void first() {
                SelectPhotoUtil.shootAvatar(getActivity(), new OnResultCallbackListener<LocalMedia>() {
                    ...
                    public void onResult(List<LocalMedia> result) {
                        ...
                        client.upadteCustomerAvatar(ToolUtil.getUsername(getActivity()), file, new BaseObserver<BaseModel<String>>(getContext()) {
                            @Override
    ...
    public void changephone() {
        ...
        DialogInputExtKt.input(dialog, null, null, null, null, InputType.TYPE_CLASS_PHONE, 11, true, false, (materialDialog, text) -> {
            if (ToolUtil.checkMobileNumber(text.toString())) {
                client.updateCustomerPhone(ToolUtil.getUsername(getActivity()), text.toString(), new BaseObserver<BaseModel<String>>(getContext()) {
                    @Override
                    protected void showDialog() {
    ...
    public void changeqq() {
        ...
        DialogInputExtKt.input(dialog, null, null, null, null, InputType.TYPE_CLASS_PHONE, 10, true, false, (materialDialog, text) -> {
            client.updateCustomerQQ(ToolUtil.getUsername(getActivity()), text.toString(), new BaseObserver<BaseModel<String>>(getContext()) {
                @Override
                protected void showDialog() {
    ...
    public void onStart() {
        super.onStart();
        client.selectCustomerByUsername(ToolUtil.getUsername(getActivity()), new BaseObserver<BaseModel<Customer>>(getContext()) {
            @Override
            protected void showDialog() {
            ...
            protected void successful(BaseModel<Customer> customerBaseModel) {
                customer = customerBaseModel.getData();
                Glide.with(getContext())
                        .load(RequestAPI.baseImageURL + customer.getAvatar())
                        .apply(RequestOptions.bitmapTransform(new CircleCrop()))
                        .skipMemoryCache(true)
                        .diskCacheStrategy(DiskCacheStrategy.NONE)
                        .into(avatar_img);
                nickname.setText(customer.getUsername());
                phonenumber.setText(customer.getPhone());
                qqnumber.setText(customer.getQQ());
            }
