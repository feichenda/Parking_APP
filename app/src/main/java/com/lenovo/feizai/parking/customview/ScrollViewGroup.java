package com.lenovo.feizai.parking.customview;
...
/**
 * @author feizai
 * @date 12/31/2020 031 7:19:35 PM
 */
public class ScrollViewGroup extends ViewGroup {
    ...
    public ScrollViewGroup(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        final TypedArray array = context.getTheme().obtainStyledAttributes(attrs
                , R.styleable.ScrollViewGroup
                , defStyleAttr, 0);
        mHeaderResId = array.getResourceId
                (R.styleable.ScrollViewGroup_header_id, -1);
        mTargetResId = array.getResourceId
                (R.styleable.ScrollViewGroup_target_id, -1);
        mInnerScrollId = array.getResourceId
                (R.styleable.ScrollViewGroup_inn_id, -1);
        ...
        mHeaderInitTop = DensityUtil.dip2px(c, array.getInt(
                R.styleable.ScrollViewGroup_header_init_top, 0));
        mHeaderCurrTop = mHeaderInitTop;
        // 屏幕高度 - 底部距离 - 状态栏高度
        mTargetInitBottom = DensityUtil.dip2px(c, array.getInt(
                R.styleable.ScrollViewGroup_target_init_bottom, 0));
        // 注意：当前activity默认去掉了标题栏
        mTargetInitTop = DensityUtil.getSreenHeight(c) - mTargetInitBottom
                - DensityUtil.getStatusBarHeight(c);
